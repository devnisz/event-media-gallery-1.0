...
export async function PATCH(
  ...
  try {
    ...
    const body = (await request.json()) as { allowLikes?: unknown };
    ...
    const updateResult = await updateEventLikesSettings(
      trimmedEventId,
      body.allowLikes,
    ).catch((err: unknown) => {
      const message =
    ...
    console.error("[LIKES_CONFIG] erro", error);
